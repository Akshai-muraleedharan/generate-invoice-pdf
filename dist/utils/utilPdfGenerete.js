...
const pdfGenerate = (res, Invoice) => {
    const productsSubTotal = Invoice.products.reduce((accumulator, currentValue) => accumulator + currentValue.netAmount, 0);
    const doc = new pdfkit_1.default({ size: 'A5', margins: { top: 20, bottom: 20, left: 20, right: 20 }, });
    doc.pipe(res);
    doc.moveDown();
    doc.moveDown();
    ...
    Invoice.products.forEach((p) => {
        ...
        let gstAmount = (p.netAmount * p.gstRate) / 100;
        let gstAmtWithTotal = p.netAmount;
        doc.text(index, 30, y);
